...
public:
	...
	__declspec(dllexport) size_t			stotypesize() const; // per sapere quanto e'grande un dato descrittore sul file
	__declspec(dllexport) size_t			memtypesize() const; // per sapere quanto e'grande un dato descrittore in memoria
	__declspec(dllexport) const char *memtypename() const; 
	__declspec(dllexport) const char *stotypename() const;
};
...
{
public:
	__declspec(dllexport) inline PlyProperty()
	{
		tipo		= 0;
		islist		= 0;
		tipoindex	= 0;
	...
	__declspec(dllexport) inline PlyProperty( const char * na, int ti, int isl, int t2 )
	{
		assert(na);
		assert(ti>0);
		assert(ti<T_MAXTYPE);
...
public:
	...
	__declspec(dllexport) inline PlyElement()
	{
		number	= 0;
	}

	__declspec(dllexport) inline PlyElement( const char * na, int nu )
	{
		assert(na);
		assert(nu>=0);
	...
	__declspec(dllexport) inline void SetName( const char * na )
	{
		name = std::string(na);
	}

	__declspec(dllexport) inline void SetNumbert( int nu )
	{
		assert(nu>0);
		number = nu;
	}

	__declspec(dllexport) void AddProp( const char * na, int ti, int isl, int t2 );

	__declspec(dllexport) int AddToRead(
		const char * propname,
		int	stotype1,
		int memtype1,
		size_t offset1,
	...
	__declspec(dllexport) PlyProperty * FindProp( const char * name );
...
{
private:
	__declspec(dllexport) void compile( PlyElement * e );
	__declspec(dllexport) void compile( PlyProperty * p );
...
public:
	...
	__declspec(dllexport) PlyFile();
	__declspec(dllexport) ~PlyFile();

		// Apre un file ply
	__declspec(dllexport) int Open( const char * filename, int mode );
		// Chiude un file e disalloca la memoria
	__declspec(dllexport) void Destroy();
		// Ritorna il codice dell'ultimo errore
	__declspec(dllexport) inline int GetError() const { return error; }
	...
	__declspec(dllexport) inline int AddToRead( const PropDescriptor & p )
	{
	...
	__declspec(dllexport) const char * ElemName( int i );

	__declspec(dllexport) int ElemNumber( int i ) const;
		// Setta il tipo di elemento corrente per effetture
		// la lettura
	__declspec(dllexport) inline void SetCurElement( int i )
	{
	...
	__declspec(dllexport) int Read( void * mem );
  ...
  std::vector<PlyElement> elements;	// Vettore degli elementi
	...
	__declspec(dllexport) inline const char * GetHeader() const { return header; }
protected:
	...
	__declspec(dllexport) int OpenRead( const char * filename );
	__declspec(dllexport) int OpenWrite( const char * filename );
	
	__declspec(dllexport) PlyElement * AddElement( const char * name, int number );
	__declspec(dllexport) int FindType( const char * name ) const;
	__declspec(dllexport) PlyElement * FindElement( const char * name );
};

__declspec(dllexport) void interpret_texture_name(const char*a, const char*fn, char*output);
