...
{
public:
	__declspec(dllexport) Skeletonization(SkeleParameterSet* _para)
	{
	samples = NULL;
	original = NULL;
	skeleton = NULL;
	...
	__declspec(dllexport) ~Skeletonization(void);
public:

	__declspec(dllexport) void run();
	__declspec(dllexport) void setInput(DataMgr* pData);
	__declspec(dllexport) SkeleParameterSet* getSkeleParameterSet(){ return para; }
	__declspec(dllexport) void setSkeleParameterSet(SkeleParameterSet* _para){para = _para;}
	__declspec(dllexport) Skeleton* getSkeleton(){ return skeleton; }
	__declspec(dllexport) void clear();

	__declspec(dllexport) void setFirstIterate();
	__declspec(dllexport) int getIterateNum(){ return nTimeIterated; };
	__declspec(dllexport) double getErrorX(){return error_x;};
private:
