...
void Skeletonization::run()
{
  is_skeleton_locked = false;
	...
	if (para->getBool("Step2 Search New Branches"))
	{
...
void Skeletonization::runAutoWlopOneStep()
{
          ...
          current_radius > stop_radius)
    {
      finalProcess();
		...
		{
			para->setBool("The Skeletonization Process Should Stop", true);
		}
...
void Skeletonization::runStep0_WLOPIterationAndBranchGrowing()
{
  ...
  para->setDouble("Current Movement Error", iterate_error);
...
{
	cout << "Skeletonization destroyed!! " << endl; 
}
...
double Skeletonization::wlopIterate()
{
	...
	para->setDouble("Current Movement Error", error_x);
...
Branch Skeletonization::searchOneBranchFromDirection(int begin_idx, Point3f head_direction)
{
	...
	//double MAX_Euler_dist = 1;
	//double MAX_Perpendicular_dist = 1;
	//double MAX_Too_Close_dist = 1;
	...
	while(is_tail_growing)
	{
		...
		else
		{
			...
			if (angle0 >= 0 && angle1 >= 0 &&
				 angle0  < angle_threshold && angle1 < angle_threshold)
			{
...
void Skeletonization::increaseRadius()
{
  ...
  para->setDouble("CGrid Radius", current_radius);
...
void Skeletonization::dealWithVirtualsForTail(Branch& branch)
{
  ...
  //bool use_virtual_group_strategy = 1;
...
void Skeletonization::runAllSegment()
{
  ...
  para->setBool("Need Segment Right Away", need_remember);
}
